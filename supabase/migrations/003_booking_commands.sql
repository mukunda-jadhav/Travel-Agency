-- Apply after 002 to enable persistent demo cancellation.
...
begin
  ...
  if p_booking is not null then
    ...
    on conflict(booking_id) do update set record=excluded.record
    where travel_demo_bookings.user_phone=p_user;
  end if;
