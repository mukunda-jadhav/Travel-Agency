...
export function localIntent(input:string,s:Session):Extraction {
  ...
  if(/^(cancel|stop|never mind|nevermind)(?:\s.*)?$/i.test(t))return {...x,intent:'cancel'};
  ...
  const from=t.match(new RegExp('\\bfrom\\s+(.+?)\\s+to\\s+(.+?)'+end,'i'));
  if(from){x.criteria[type==='cab'?'pickup':'origin']=from[1]!.trim();x.criteria.destination=from[2]!.trim()}
  else {const to=t.match(new RegExp('\\bto\\s+(?!fly\\b|travel\\b|book\\b)(.+?)'+end,'i'));if(to)x.criteria.destination=to[1]!.trim()}
  if(!from){const origin=t.match(new RegExp('\\bfrom\\s+(.+?)'+end,'i'));if(origin)x.criteria[type==='cab'?'pickup':'origin']=origin[1]!.trim()}
  ...
  if(s.pendingField&&x.intent==='unknown'){
    ...
    if(s.pendingField==='fullName'){if(/^[\p{L}][\p{L} .'-]{2,100}$/u.test(t))x.details.fullName=t.replace(/^(?:my name is|i am)\s+/i,'')}
    else if(s.pendingField==='email'){if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t))x.details.email=t}
    else if(!Object.keys(x.criteria).length)Object.assign(x.criteria,{[s.pendingField]:t});
  }
  if(s.status==='collecting_booking_details'){
    const email=t.match(/[^\s,]+@[^\s,]+\.[^\s,]+/);if(email)x.details.email=email[0];
    const name=t.match(/(?:my name is|i am)\s+([\p{L} .'-]+?)(?=,|\s+and\b|$)/iu);if(name)x.details.fullName=name[1]!.trim();
  }
  if(x.intent==='unknown'&&Object.keys(x.criteria).length)x.intent='modify_search';
