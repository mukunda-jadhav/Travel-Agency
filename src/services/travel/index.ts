...
export function createTravelAssistant(store:TravelStore,dependencies={extractIntent,providers},clock=()=>new Date()) {
  ...
  return (userId:string,input:string,eventId:string):Promise<string>=>{
    const result=queue.catch(()=>undefined).then(async()=>{
      if(!/^[\w:+.-]{3,100}$/.test(userId)||!eventId||eventId.length>250||!input.trim()||input.length>4000)throw new Error('Invalid travel message');
      const cached=await store.reply(userId,eventId);if(cached!==undefined)return cached;
      ...
      await store.commit(userId,eventId,turn);return turn.reply;
    });queue=result;return result;
  };
}
export const runTravelAssistant=createTravelAssistant(config.DEMO_MODE?new FileTravelStore(config.TRAVEL_DATA_FILE):new SupabaseTravelStore());
