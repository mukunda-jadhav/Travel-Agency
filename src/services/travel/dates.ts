...
export function resolveDate(value:string,today:string):string|undefined {
  ...
  const named=s.match(/^(\d{1,2})(?:st|nd|rd|th)?\s+(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+(\d{4}))?$/i);
  if(named){
...
export const datePhrase=/\b\d{1,2}(?:st|nd|rd|th)?\s+(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+\d{4})?\b|\d{4}-\d{2}-\d{2}|day after tomorrow|tomorrow(?: morning| evening| night)?|today|tonight|(?:this |next )?weekend|(?:this |next )?(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)/ig;
