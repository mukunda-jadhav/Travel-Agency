...
function friendlyError(e: unknown) {
  ...
  if (/quota|429|too many requests/i.test(message))
    return "The travel assistant is temporarily at its AI usage limit. Please try again later.";
  if (/Duffel Stays is not enabled/i.test(message))
    return "Hotel booking is not enabled on our supplier account yet. I can still help with flights and cab estimates.";
...
async function processMessage(m: { id: string; from: string; text: string }) {
  ...
  try {
    ...
    const cancel = m.text.match(
      /^(?:\/cancel|cancel booking)\s+([0-9a-f-]{36})$/i,
    );
    if (
      !config.DEMO_MODE &&
      (status || cancel) &&
      !(await claimEvent("meta", m.id))
    )
      return;
    const reply =
      !config.DEMO_MODE && status
        ? await statusText(status[1]!, m.from)
...
function enqueueMessage(m: { id: string; from: string; text: string }) {
  const prior = userQueues.get(m.from) ?? Promise.resolve();
  const next = prior
    .catch(() => undefined)
  ...
  userQueues.set(m.from, next);
}
export async function whatsappWebhook(req: Request, res: Response) {
  const raw = (req as Request & { rawBody?: Buffer }).rawBody;
  const signatureValid =
    !!raw &&
    validMetaSignature(raw, req.header("x-hub-signature-256") ?? undefined);
  const messages = messagesFrom(req.body);
